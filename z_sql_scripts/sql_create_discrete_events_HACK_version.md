...
/**********************
Pre Step: Change the Schema
**********************/

-- CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_0pre_list_dates` AS
-- with date_parts as (
--   SELECT
--       distinct
--       datetime
--     , DATE_ADD('1900-01-01', INTERVAL cast(datetime as int64) day) as date
--     , cast(floor((datetime - floor(datetime)) * 24) as int64) as hour
--     , cast(round((((datetime - floor(datetime)) * 24) - floor(((datetime - floor(datetime)) * 24))) * 60,0) as int64) as minute
--   FROM `hca-cti-ds-hackathon.f4_fhm.test_load_400_patients_1`
-- )
-- SELECT datetime, cast(cast(date || 'T' || hour || ':' || minute || ':00' as datetime) as timestamp) as datetime_update
-- FROM date_parts
-- ;

CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_0pre_force_schema` AS
with clean_up as (
  SELECT *
  FROM `hca-cti-ds-hackathon.f4_fhm.test_load_400_patients_1`
  WHERE left(datetime,2) = '20'
),
unpivot_function as (
  SELECT data_value_241 as subjectid, cast(datetime as timestamp) as measurement_timestamp, datatype, MonitorID, sensortype, key, value
  FROM clean_up
  UNPIVOT(
    value for key in (
        data_value_1
...
time_add_key_x as (
  SELECT
    ...
    , safe_cast(right(cast(key as string),length(cast(key as string))-length("data_value_")) as int64) as time_add_key
  FROM unpivot_function
),
milliseconds_add_x as (
  SELECT
    subjectid
    , measurement_timestamp
    , datatype
    ...
    , sensortype
    , value
    , cast(floor((time_add_key - 1) / 4) as int64) as seconds_add
    , cast(mod(time_add_key-1,4)*25 as int64) as milliseconds_add
  FROM time_add_key_x
)
SELECT subjectid, TIMESTAMP_ADD(TIMESTAMP_ADD(measurement_timestamp, INTERVAL seconds_add SECOND), INTERVAL milliseconds_add MILLISECOND) AS measurement_timestamp, datatype, MonitorID, sensortype, cast(value as float64) as value
FROM milliseconds_add_x
;
...
CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_0_union_all_input_data` AS
SELECT * FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_0pre_force_schema`
UNION ALL
SELECT * FROM `hca-cti-ds-hackathon.f4_fhm.synthetic_10_change_to_schema`
;

# Note: this step should narrow down to just the last ~60 minutes or so during pipeline process in production

CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_0_summarize_by_second5` AS
with pre_table as
(
  SELECT
      subjectid
    ...
    , avg(value) as value
  -- SELECT *
  FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_0_union_all_input_data` a
  GROUP BY 1,2,3
)
SELECT
    subjectid
...
CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_1_baseline_last_30_min` AS
WITH array_summary_last_30_min as
  (
    ...
    (
      ...
      FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_0_summarize_by_second5`
    ) a
  )
...
CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_2_variability_last_3_min` AS
with prior_value AS
(
  SELECT
...
(
  SELECT
    ...
    , b.cycle_length
    , b.amplitude
  FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_1_baseline_last_30_min` a
...
CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_3_events_part1` AS
...
with is_event as (
  ...
  *
  ...
  FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_2_variability_last_3_min`
),
...
CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_3_events_part2` AS
...
with period_state_every_y_seconds_event_id as (
  ...
  FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_3_events_part1` a
  ...
  (
    ...
    FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_3_events_part1`
...
CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_3_events_part3` AS
...
FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_3_events_part2` a
LEFT JOIN
(
  SELECT *
  FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_3_events_part2`
...
CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_4_contraction` AS
...
(
  ...
  FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_3_events_part3`
...
(
  ...
  FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_2_variability_last_3_min` a
...
CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_4_deceleration` AS
...
(
  ...
  LEFT JOIN `hca-cti-ds-hackathon.f4_fhm.rules_classify_4_contraction` b
    ON a.subjectid = b.subjectid
...
(
  ...
  LEFT JOIN dec_con_mapping b
    ON a.subjectid = b.subjectid
    AND a.deceleration_event_id = b.deceleration_event_id
  LEFT JOIN `hca-cti-ds-hackathon.f4_fhm.rules_classify_4_contraction` c
    ON a.subjectid = c.subjectid
    AND b.contraction_event_id = c.contraction_event_id
...
CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_4_acceleration` AS
...
CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_4_uterine_stimulation` AS
...
CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_5_summary_table_pre` AS
...
CREATE OR REPLACE TABLE `hca-cti-ds-hackathon.f4_fhm.rules_classify_5_summary_table` AS
SELECT
    a.*
...
FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_5_summary_table_pre` a
LEFT JOIN
(
  SELECT distinct
      a.measurement_timestamp
    ...
    , b.seconds_overlap
  FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_5_summary_table_pre` a
  CROSS JOIN `hca-cti-ds-hackathon.f4_fhm.rules_classify_4_deceleration` b
  WHERE a.subjectid = b.subjectid
...
(
  ...
  FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_5_summary_table_pre` a
  CROSS JOIN `hca-cti-ds-hackathon.f4_fhm.rules_classify_4_contraction` b
  WHERE a.subjectid = b.subjectid
...
(
  ...
  FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_5_summary_table_pre` a
  CROSS JOIN `hca-cti-ds-hackathon.f4_fhm.rules_classify_4_acceleration` b
  WHERE a.subjectid = b.subjectid
...
(
  SELECT distinct
    ...
    , b.seconds_overlap
  FROM `hca-cti-ds-hackathon.f4_fhm.rules_classify_5_summary_table_pre` a
  CROSS JOIN `hca-cti-ds-hackathon.f4_fhm.rules_classify_4_uterine_stimulation` b
  WHERE a.subjectid = b.subjectid
